package pl.caltonek.chatFusion.util;
...
public final class ColorUtil {
    ...
    public static @NotNull String stripFormatting(final @Nullable String input) {
        if (input == null || input.isEmpty()) return "";

        return input
                .replaceAll("(?i)&#[0-9a-fA-F]{6}>(.*?)&#[0-9a-fA-F]{6}<", "$1")
                .replaceAll("(?i)\\{#[0-9a-fA-F]{6}\\}>(.*?)\\{#[0-9a-fA-F]{6}<\\}", "$1")
                .replaceAll("(?i)\\{#rainbow\\}(.*?)\\{/#rainbow\\}", "$1")
                .replaceAll("(?i)&#[0-9a-fA-F]{6}", "")
                .replaceAll("(?i)\\{#[0-9a-fA-F]{6}\\}", "")
                .replaceAll("(?i)<#[0-9a-fA-F]{6}>", "")
                .replaceAll("(?i)[&§][0-9a-fk-orK-OR]", "")
                .replaceAll("(?i)&u", "")
                .replaceAll("<[^>]*>", "");
    }
    ...
    public static @NotNull String parsePatterns(final @Nullable String str) {
        ...
        result = result.replaceAll("(?i)\\{#rainbow\\}", "<rainbow>")
